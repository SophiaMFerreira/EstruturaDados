/* Complexidade O(?)
    ...
    11. Qual foi o valor médio encontrado pelo seu programa? 49.58
...
main(){
    ...
    printf("Complex med: %.2f", complexidadeMedia(apontaVetComplex, apontaVetor));
}

int verificaOcorrenciaVetor(int intRand, int *apontaVetor, int limite){
    for(int j = 0; j < limite; j++){
        if(intRand == *(apontaVetor + j)){
            return 1; // Se retornar J (a posição de ocorrencia) não funciona
        }
    }
    return 0;
}
